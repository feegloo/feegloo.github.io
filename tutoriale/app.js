...
(() => {
  ...
  // Śledzenie sekcji bez przewijania strony przy zmianie adresu.
  ...
  const sections = [...document.querySelectorAll('aside nav a[href^="#"]')]
    .map(link => ({ link, target: document.getElementById(fragmentId(link.getAttribute('href'))) }))
  ...
  function markFromAddress() {
    ...
    markActive(sections.find(item => item.target.id === id) || (!id ? sections[0] : null));
  }
  function updateFromScroll() {
    const visible = sections.filter(item => item.target.getClientRects().length);
    if (!visible.length) return;
    const readingLine = Math.max(100, window.innerHeight * 0.3);
    let active = visible[0];
    for (const item of visible) {
      if (item.target.getBoundingClientRect().top <= readingLine) active = item;
    }
    // Krótka ostatnia sekcja również może stać się aktywna.
    ...
    if (fragmentId(window.location.hash) !== active.target.id) {
      ...
      } catch { /* Niektóre przeglądarki ograniczają history dla file://. */ }
    }
  }
